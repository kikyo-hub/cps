<template>
  <div class="part3" id="pdfCentent">
    ...
    <template v-if="monitordata">
      <p style="font-size:18px">1.  {{ monitordata[4].monitorName }}</p>
      <p style="font-size:18px">通过布控在客户的监测点：{{ monitordata[4].monitorName }}，{{ monitorDesc }}</p>
      <div id="KWH" style="width:90%"></div>
      <p align="center" style="font-size:18px"> 图3.3 {{ monitordata[4].monitorName }}用电量曲线图</p>
      <p style="font-size:18px">2.  {{ monitordata[5].monitorName }}</p>
      <p style="font-size:18px">通过布控在客户的监测点：{{ monitordata[5].monitorName }}，{{ monitorDesc1 }}</p>
      <div id="KWH1" style="width:90%"></div>
      <p align="center" style="font-size:18px"> 图3.5 {{ monitordata[5].monitorName }}用电量曲线图</p>
    </template>
  </div>
</template>
...
export default {
  ...
  created () {
    ...
    getOrgTree().then(res => {
      ...
      this.treekey = this.monitordata[4].monitorId
      this.treekey1 = this.monitordata[5].monitorId
      switch (parseInt(this.monitordata[4].deviceType)) {
        case 1:
      ...
      switch (parseInt(this.monitordata[5].deviceType)) {
        case 1:
